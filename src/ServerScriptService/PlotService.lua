...
local function buildStadiumBowl(plot, origin, level)
	...
	local rows = math.clamp(level, 1, 5)        -- grows with the Lower Stands level
...
function PlotService.buildPlot(player)
	...
	for _, buildingCfg in ipairs(BuildingConfig.Buildings) do
		...
		-- pitch/lowerstands markers don't scale (the field/bowl is built separately).
		if buildingCfg.id ~= "stands" and buildingCfg.id ~= "pitch"
			and buildingCfg.id ~= "lowerstands" and level > 0 then
