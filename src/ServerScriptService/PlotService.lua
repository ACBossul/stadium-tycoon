...
local BUILDING_LAYOUT = {
	pitch       = { x = -16, z = -30 },   -- "Build the Pitch" terminal (centre-south)
	lowerstands = { x =  16, z = -30 },   -- "Build Lower Stands" terminal (centre-south)
	structure   = { x =   0, z =  42 },   -- "Stadium Roof" terminal (centre, by the grandstand)
...
local BUILDING_PIECES = {
	-- "Build" terminals for the progressive structures (pitch + lower stands). The
	-- terminal is the clickable base that carries the buy/upgrade billboard; the
	-- actual field/bowl is generated separately when the level is >= 1.
	pitch = {
		{ size = Vector3.new(6, 4.5, 6), offset = Vector3.new(0, 2.25, 0), color = Color3.fromRGB(70, 130, 80),  material = M.SmoothPlastic, collide = true },
		{ size = Vector3.new(7, 0.6, 7), offset = Vector3.new(0, 4.7, 0),  color = Color3.fromRGB(245, 230, 120), material = M.Neon, collide = false },
	},
	lowerstands = {
		{ size = Vector3.new(6, 4.5, 6), offset = Vector3.new(0, 2.25, 0), color = Color3.fromRGB(70, 96, 150),  material = M.SmoothPlastic, collide = true },
		{ size = Vector3.new(7, 0.6, 7), offset = Vector3.new(0, 4.7, 0),  color = Color3.fromRGB(150, 200, 255), material = M.Neon, collide = false },
	},
	structure = {
		{ size = Vector3.new(6, 4.5, 6), offset = Vector3.new(0, 2.25, 0), color = Color3.fromRGB(120, 96, 60),  material = M.SmoothPlastic, collide = true },
		{ size = Vector3.new(7, 0.6, 7), offset = Vector3.new(0, 4.7, 0),  color = Color3.fromRGB(255, 200, 110), material = M.Neon, collide = false },
	},
...
local function buildSurrounds(plot, origin)
	...
	local function wall(cx, cz, sx, sz)
		...
		if horizontal then
			canopy.Size     = Vector3.new(sx + 4, 2.5, 10)
			canopy.Position = origin + Vector3.new(cx, wallH + 1.25, cz - sgnZ * 4)
		else
			canopy.Size     = Vector3.new(10, 2.5, sz + 4)
			canopy.Position = origin + Vector3.new(cx - sgnX * 4, wallH + 1.25, cz)
		end
		canopy.Parent = plot
	...
	-- East wall split to leave an ~18-stud KART EXIT near the parking lot (z ≈ -40).
	wall( half,  29.5, wallT, 121)       -- east wall, north of the kart gap
	wall( half, -69.5, wallT, 41)        -- east wall, south of the kart gap
...
-- The EAST kart exit, dressed as a real entrance: an arch (pillars + lintel) + a
-- force-field gate with the same owner open/close button as the main entrance.
local function buildKartGate(plot, origin, player)
	local gx, gz, gw = PLOT_SIZE.X / 2, -40, 18   -- east wall, kart-gap centre, width

	-- Arch: two pillars + a lintel framing the opening.
	local function pillar(z)
		local p = Instance.new("Part")
		p.Anchored = true; p.Size = Vector3.new(4, 13, 4)
		p.Position = origin + Vector3.new(gx, 6.5, z)
		p.Color = Color3.fromRGB(88, 92, 106); p.Material = Enum.Material.Concrete; p.Parent = plot
	end
	pillar(gz - gw / 2); pillar(gz + gw / 2)
	local lintel = Instance.new("Part")
	lintel.Anchored = true; lintel.Size = Vector3.new(4, 3, gw + 6)
	lintel.Position = origin + Vector3.new(gx, 12, gz)
	lintel.Color = Color3.fromRGB(70, 74, 86); lintel.Material = Enum.Material.Concrete; lintel.Parent = plot
	local sign = Instance.new("Part")
	sign.Anchored = true; sign.CanCollide = false; sign.Size = Vector3.new(0.4, 2.4, gw)
	sign.Position = origin + Vector3.new(gx - 2.2, 12, gz)
	sign.Color = Color3.fromRGB(20, 22, 30); sign.Material = Enum.Material.SmoothPlastic; sign.Parent = plot
	local sg = Instance.new("SurfaceGui"); sg.Face = Enum.NormalId.Front
	sg.SizingMode = Enum.SurfaceGuiSizingMode.PixelsPerStud; sg.PixelsPerStud = 30; sg.LightInfluence = 0; sg.Parent = sign
	local sl = Instance.new("TextLabel"); sl.Size = UDim2.new(1,0,1,0); sl.BackgroundTransparency = 1
	sl.Text = "🚗 KART EXIT"; sl.TextColor3 = Color3.fromRGB(120,230,255); sl.TextScaled = true; sl.Font = Enum.Font.GothamBlack; sl.Parent = sg

	-- Force-field gate (kill-curtain for non-owners, like the main entrance).
	local door = Instance.new("Part")
	door.Name = "BaseDoor"; door.Anchored = true; door.CanCollide = false
	door.Size = Vector3.new(1, 8, gw); door.Position = origin + Vector3.new(gx, 4, gz)
	door.Material = Enum.Material.ForceField; door.Parent = plot
	door:SetAttribute("Closed", true)
	local function paintDoor()
		local closed = door:GetAttribute("Closed")
		door.Color = closed and Color3.fromRGB(220,60,60) or Color3.fromRGB(70,210,110)
	...
	door.Touched:Connect(function(hit)
		...
		if not hum or hum.Health <= 0 then return end
		local toucher = Players:GetPlayerFromCharacter(char)
		if not toucher or toucher == player then return end
		hum.Health = 0
	end)

	local button = Instance.new("Part")
	button.Name = "DoorButton"; button.Anchored = true; button.Size = Vector3.new(4, 2, 4)
	button.Position = origin + Vector3.new(gx - 9, 1, gz + 11)
	button.Material = Enum.Material.Neon; button.Parent = plot
	local cd = Instance.new("ClickDetector"); cd.MaxActivationDistance = 20; cd.Parent = button
	local bb = Instance.new("BillboardGui"); bb.Size = UDim2.new(0,150,0,32); bb.StudsOffset = Vector3.new(0,2.2,0)
	bb.AlwaysOnTop = true; bb.MaxDistance = 60; bb.Adornee = button; bb.Parent = button
	local lbl = Instance.new("TextLabel"); lbl.Size = UDim2.new(1,0,1,0); lbl.BackgroundTransparency = 1
	lbl.TextScaled = true; lbl.Font = Enum.Font.GothamBold; lbl.TextColor3 = Color3.new(1,1,1); lbl.Parent = bb
	local function paintButton()
		local closed = door:GetAttribute("Closed")
		button.Color = closed and Color3.fromRGB(220,60,60) or Color3.fromRGB(70,210,110)
		lbl.Text = closed and "🚪 Gate: CLOSED" or "🚪 Gate: OPEN"
	end
	paintButton()
	cd.MouseClick:Connect(function(clicker)
		if clicker ~= player then return end
		door:SetAttribute("Closed", not door:GetAttribute("Closed"))
		paintDoor(); paintButton()
	end)
end
...
local function buildCashStand(plot, origin, player)
	local pos = origin + Vector3.new(-58, 0, -50)   -- SW concourse, clear of the city-teleport pad
...
-- The Tap-to-Earn button: at the very start (nothing built, no income) you tap
-- this for coins to afford your first buildings. As passive income grows it
-- becomes a small bonus — the game automates itself. Main wires the clicks.
local function buildTapButton(plot, origin, player)
	local pos = origin + Vector3.new(0, 0, -64)   -- centre-south, in the entrance path

	local base = Instance.new("Part")
	base.Name = "TapButton"; base.Anchored = true; base.CanCollide = true
	base.Size = Vector3.new(8, 2, 8)
	base.Position = pos + Vector3.new(0, 1, 0)
	base.Color = Color3.fromRGB(40, 44, 56); base.Material = Enum.Material.Metal
	base.TopSurface = Enum.SurfaceType.Smooth; base.Parent = plot

	local btn = Instance.new("Part")   -- the glowing "button" cap
	btn.Name = "Cap"; btn.Anchored = true; btn.CanCollide = false; btn.Shape = Enum.PartType.Cylinder
	btn.Size = Vector3.new(1.4, 6, 6)
	btn.CFrame = CFrame.new(pos + Vector3.new(0, 2.6, 0)) * CFrame.Angles(0, 0, math.rad(90))
	btn.Color = Color3.fromRGB(95, 230, 130); btn.Material = Enum.Material.Neon
	btn.Parent = plot
	...
	local cd = Instance.new("ClickDetector")
	cd.Name = "TapClick"; cd.MaxActivationDistance = 20; cd.Parent = base

	local bb = Instance.new("BillboardGui")
	bb.Size = UDim2.new(0, 210, 0, 64); bb.StudsOffset = Vector3.new(0, 4.5, 0)
	bb.AlwaysOnTop = true; bb.MaxDistance = 220; bb.Adornee = base; bb.Parent = base
	local lbl = Instance.new("TextLabel")
	lbl.Size = UDim2.new(1, 0, 1, 0); lbl.BackgroundTransparency = 1
	lbl.Text = "💰 TAP TO EARN\nclick for coins!"
	lbl.TextColor3 = Color3.fromRGB(140, 255, 170); lbl.TextScaled = true
	lbl.Font = Enum.Font.GothamBlack; lbl.TextStrokeTransparency = 0.4; lbl.Parent = bb
...
local function buildStadiumBowl(plot, origin, level)
	-- Re-runnable: clear old bowl parts so it regrows on each Lower Stands upgrade.
	for _, c in ipairs(plot:GetChildren()) do
		if c.Name == "BowlPart" then c:Destroy() end
	end
...
-- The end-game stadium structure: facade panels raise the four walls, and a roof
-- ring grows inward + upward each level, evolving the lot into an enclosed stadium
-- (a central opening stays over the pitch). Re-runnable; level 0 just clears.
local function buildStadiumStructure(plot, origin, level)
	for _, c in ipairs(plot:GetChildren()) do
		if c.Name == "StadiumStruct" then c:Destroy() end
	end
	...
	local function sp(sx, sy, sz, x, y, z, color, mat)
		local p = Instance.new("Part")
		p.Name = "StadiumStruct"; p.Anchored = true; p.CanCollide = true
		p.Size = Vector3.new(sx, sy, sz); p.Position = origin + Vector3.new(x, y, z)
		p.Color = color; p.Material = mat or Enum.Material.Concrete
		p.TopSurface = Enum.SurfaceType.Smooth; p.BottomSurface = Enum.SurfaceType.Smooth
		p.Parent = plot
	...
	-- Facade panels raising the four exterior walls (above the lot wall; the
	-- entrance gaps stay open below, so these read as arches over the gates).
	sp(PLOT_SIZE.X, FAC_H, 2, 0, 10 + FAC_H / 2,  half, FAC_C)
	sp(PLOT_SIZE.X, FAC_H, 2, 0, 10 + FAC_H / 2, -half, FAC_C)
	sp(2, FAC_H, PLOT_SIZE.Z,  half, 10 + FAC_H / 2, 0, FAC_C)
	sp(2, FAC_H, PLOT_SIZE.Z, -half, 10 + FAC_H / 2, 0, FAC_C)
	-- Corner pylons rising to the roof.
	for _, sx in ipairs({ -1, 1 }) do
		for _, sz in ipairs({ -1, 1 }) do
			sp(4, roofY, 4, sx * (half - 3), roofY / 2, sz * (half - 3), PYL_C, Enum.Material.Metal)
		end
	end
	-- Roof ring: four cantilevered slabs leaving a central opening over the pitch.
	sp(PLOT_SIZE.X, 2, roofW, 0, roofY,  half - roofW / 2, ROOF_C, Enum.Material.Metal)
	sp(PLOT_SIZE.X, 2, roofW, 0, roofY, -half + roofW / 2, ROOF_C, Enum.Material.Metal)
	local sideLen = PLOT_SIZE.Z - 2 * roofW
	if sideLen > 4 then
		sp(roofW, 2, sideLen,  half - roofW / 2, roofY, 0, ROOF_C, Enum.Material.Metal)
		sp(roofW, 2, sideLen, -half + roofW / 2, roofY, 0, ROOF_C, Enum.Material.Metal)
	end
end
...
-- The pitch + hoardings + animated players live in one folder so they can be
-- built once (on first purchase) and wiped cleanly (e.g. on a rebirth reset).
local function buildPitchComplex(plot, origin)
	if plot:FindFirstChild("PitchComplex") then return end
	local folder = Instance.new("Folder")
	folder.Name = "PitchComplex"
	folder.Parent = plot
	buildPitch(folder, origin)
	buildHoardings(folder, origin)
	buildPitchPlayers(folder, origin)
end
local function clearPitchComplex(plot)
	local f = plot:FindFirstChild("PitchComplex")
...
function PlotService.buildPlot(player)
	...
	-- The perimeter wall + entrance gate are always present (the lot itself). The
	-- pitch, hoardings and lower-stands bowl are PROGRESSIVE — built only once the
	-- player has purchased them (handled after the building loop, using data levels).
	buildSurrounds(plot, origin)
	buildDoor(plot, origin, player)
	buildKartGate(plot, origin, player)
	...
	for _, buildingCfg in ipairs(BuildingConfig.Buildings) do
		...
		-- The structural markers (pitch/lowerstands/structure) don't scale — their
		-- field/bowl/roof is built separately. Grandstand grows via buildStandTiers.
		local marker = buildingCfg.id == "pitch" or buildingCfg.id == "lowerstands" or buildingCfg.id == "structure"
		if buildingCfg.id ~= "stands" and not marker and level > 0 then
			scaleBuildingToLevel(m, level)
		end
	end

	-- Progressive structures: build the field + bowl only if their level >= 1.
	local pitchLvl = (data and data.stadium and data.stadium.pitch) or 0
	local bowlLvl  = (data and data.stadium and data.stadium.lowerstands) or 0
	local structLvl = (data and data.stadium and data.stadium.structure) or 0
	if pitchLvl  >= 1 then buildPitchComplex(plot, origin) end
	if bowlLvl   >= 1 then buildStadiumBowl(plot, origin, bowlLvl) end
	if structLvl >= 1 then buildStadiumStructure(plot, origin, structLvl) end
...
function PlotService.onBuildingUpgraded(player, buildingId, level)
	...
	elseif buildingId == "pitch" then
		-- First purchase lays the field; a reset (level 0) wipes it.
		if level >= 1 then buildPitchComplex(plot, origin) else clearPitchComplex(plot) end
	elseif buildingId == "lowerstands" then
		-- The bowl (re)builds, growing with the level.
		buildStadiumBowl(plot, origin, level)
	elseif buildingId == "structure" then
		-- The stadium facade + roof (re)builds, evolving with the level.
		buildStadiumStructure(plot, origin, level)
	else
